...
async def save_preset(
    ...
    if product_images:
        ...
        for idx, product_image in enumerate(product_images):
            ...
            # 生成文件名（统一使用 .jpg）
            image_filename = f"{sheet_id}_{idx}.jpg"
            ...
            # 存储相对路径
            relative_path = str(image_path.relative_to(Path(__file__).parent.parent.parent.parent))
    ...
    with get_db() as conn:
        ...
        existing = conn.execute(
            "SELECT id, product_image_paths FROM sheet_presets WHERE sheet_id = ?",
            (sheet_id,)
        ).fetchone()

        # 如果有新图片上传，删除旧图片
        if existing and existing["product_image_paths"] and image_paths:
            try:
                old_paths = json.loads(existing["product_image_paths"])
                for old_path in old_paths:
                    old_file = Path(__file__).parent.parent.parent.parent / old_path
                    if old_file.exists():
                        old_file.unlink()
            except Exception as e:
                print(f"[预设] 删除旧图片失败: {e}")
